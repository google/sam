...
#### 1. Claim Extraction
When an agent or user submits a valid OIDC JWT, the sam-control-plane verifies the token's signature against the Identity Provider. Once validated, the Hub extracts the payload claims—typically attributes like sub (subject), email, and custom arrays like groups or roles.
...
The sam-control-plane takes these generated Datalog facts and embeds them into the Authority Block of a brand new Biscuit token. The Hub signs this block with its private cryptographic key.
