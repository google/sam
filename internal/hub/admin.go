...
var adminLogger = golog.Logger("sam-hub-admin")
...
func HandleBan(admin HubAdmin) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		adminLogger.Infof("[Admin API] Received ban request for peer: %s", peerID)
		...
		adminLogger.Infof("[Admin API] Publishing ban event for peer: %s", peerID)
		if err := admin.PublishEvent(r.Context(), event); err != nil {
			adminLogger.Errorf("[Admin API] Failed to publish event: %v", err)
			http.Error(w, fmt.Sprintf("Failed to publish event: %v", err), http.StatusInternalServerError)
			return
		}

		adminLogger.Infof("[Admin API] Successfully published ban event for peer: %s", peerID)
