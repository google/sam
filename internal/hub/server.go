...
// HandleRegisterHTTP registers HTTP enrollment request.
func (h *Hub) HandleRegisterHTTP(w http.ResponseWriter, r *http.Request) {
	...
	h.logger.Infow("New HTTP enrollment request", "peer_id", req.PeerId)
	...
	if err != nil {
		h.logger.Errorw("JWT verification failed", "peer_id", req.PeerId, "error", err)
	...
	if err != nil {
		h.logger.Errorw("Invalid Peer ID", "peer_id", req.PeerId, "error", err)
	...
	if err != nil {
		h.logger.Errorw("Biscuit minting failed", "peer_id", req.PeerId, "error", err)
	...
	if err != nil {
		h.logger.Errorf("[Enroll] Failed to marshal response: %v", err)
	...
	h.logger.Infow("Successfully enrolled peer via HTTP", "peer_id", req.PeerId)
	samHubEnrollmentTotal.WithLabelValues("success").Inc()
}

// HandleInfoHTTP retrieves basic metadata info of the Hub.
func (h *Hub) HandleInfoHTTP(w http.ResponseWriter, r *http.Request) {
	...
	respData, err := proto.Marshal(resp)
	if err != nil {
		h.logger.Errorf("[Info] Failed to marshal info response: %v", err)
		http.Error(w, "Failed to serialize response", http.StatusInternalServerError)
		return
	}
