...
func TestHandleMCPStream_RegistersAggregatedTools(t *testing.T) {
	...
	// Manually register the hosted service on A and aggregate.
	manifest := &ServiceManifest{
		Info:    &api.ServiceInfo{Type: api.ServiceType_SERVICE_TYPE_MCP, Name: "code-reviewer"},
		Handler: handler,
	}
	if err := nodeA.aggregateServiceTools(ctx, manifest); err != nil {
		t.Fatalf("aggregateServiceTools: %v", err)
	}
	nodeA.servicesMu.Lock()
	nodeA.services["code-reviewer"] = manifest
	nodeA.servicesMu.Unlock()
	t.Cleanup(func() {
		nodeA.servicesMu.Lock()
		delete(nodeA.services, "code-reviewer")
		nodeA.servicesMu.Unlock()
		nodeA.detachServiceTools(manifest)
	})
...
func TestHandleMCPStream_ForwarderRoutesCalls(t *testing.T) {
	...
	nodeA.servicesMu.Lock()
	nodeA.services["svc"] = manifest
	nodeA.servicesMu.Unlock()
	t.Cleanup(func() {
		nodeA.servicesMu.Lock()
		delete(nodeA.services, "svc")
		nodeA.servicesMu.Unlock()
		nodeA.detachServiceTools(manifest)
	})
