...
func TestInteractiveLogin(t *testing.T) {
	...
	token, err := node.InteractiveLogin(ctx, "http://auth.example.com/auth", server.URL+"/token", "client_id_test", "sam-e2e", false)
	if err != nil {
...
func TestInteractiveLoginWithRefresh(t *testing.T) {
	...
	token, err := node.InteractiveLogin(ctx, "http://auth.example.com/auth", server.URL+"/token", "client_id_test", "sam-e2e", true)
	if err != nil {
		t.Fatalf("InteractiveLogin failed: %v", err)
	}
